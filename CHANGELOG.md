...
- **Special offers** — `client.V1.Conversations[id].SpecialOffers.PostAsync(...)`, `.SpecialOffers[offerId].GetAsync()` / `.DeleteAsync()` (`POST`/`GET`/`DELETE /v1/conversations/{id}/special-offers[/{offerId}]`), plus `client.V1.Channels.Airbnb.Offers.GetAsOffersGetResponseAsync(q => q.QueryParameters.OfferId = ...)`.
